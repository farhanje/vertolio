...
function prototypeProps(value = {}) {
  return {
    ...
    analyticsPrefix: value?.analyticsPrefix || 'portfolio_prototype',
...
const components = {
  types: {
    ...
    interactivePrototype: ({value}) => <InteractivePrototype {...prototypeProps(value)} />,
    ...
    dataVisualization: ({value}) => <DataVisualization {...dataVizProps(value)} />,
    evidenceGrid: ({value}) => (
      <EvidenceGrid
        eyebrow={value?.eyebrow}
        title={value?.title}
