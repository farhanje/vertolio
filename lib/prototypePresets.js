...
const copy = {
  en: {
    intro: ['Upgrade entry', 'Set expectations', 'The upgrade page explains the two verification tasks before the user enters KYC.'],
    hub: ['Review hub', 'Task hub replaces forced sequence', 'Choose either KTP or Selfie. The flow no longer dictates one fixed order.'],
    ktp: ['Capture KTP', 'Complete one task', 'The identity document task is completed independently from Selfie.'],
    saved: ['Autosaved progress', 'Completed work persists', 'The user returns to Review with one task retained and the other still actionable.'],
    selfie: ['Selfie / liveness', 'Resume from the remaining task', 'Only the remaining task needs attention. Previously completed work stays intact.'],
    complete: ['Ready to submit', 'Tasks converge before submission', 'Both independently completed tasks resolve into one final Review state.'],
    processing: ['Verification sent', 'Submission state', 'The flexible task model ends in the same operational review state after submission.'],
  },
  id: {
    intro: ['Masuk ke upgrade', 'Ekspektasi di awal', 'Halaman upgrade menjelaskan dua task verifikasi sebelum user masuk ke KYC.'],
    hub: ['Tinjauan', 'Hub menggantikan urutan wajib', 'User bisa mulai dari KTP atau Selfie. Flow tidak lagi mengunci satu urutan.'],
    ktp: ['Ambil foto KTP', 'Selesaikan satu task', 'Dokumen identitas dapat diselesaikan terpisah dari Selfie.'],
    saved: ['Progress tersimpan', 'Task yang selesai tetap tersimpan', 'Saat kembali ke Tinjauan, task yang sudah selesai tetap ada dan task lain masih bisa dikerjakan.'],
    selfie: ['Selfie / liveness', 'Lanjutkan dari task yang tersisa', 'User hanya perlu menyelesaikan task yang belum selesai tanpa mengulang progress sebelumnya.'],
    complete: ['Siap dikirim', 'Task bertemu sebelum submit', 'Dua task yang diselesaikan secara independen kembali ke satu Tinjauan akhir.'],
    processing: ['Verifikasi dikirim', 'Status setelah submit', 'Flow yang fleksibel tetap berakhir pada proses verifikasi operasional yang sama.'],
  },
}
...
function makeSteps(locale = 'en') {
  ...
  return [
    {key:'intro',navNumber:'01',counter:'01 / 07',label:c.intro[0],annotation:c.intro[1],caption:c.intro[2],alt:'AstraPay Upgrade Akun screen',src:screens.intro,nextKey:'hub-empty',event:'kyc_autosave_start'},
    {key:'hub-empty',navNumber:'02',counter:'02 / 07',label:c.hub[0],annotation:`01 · ${c.hub[1]}`,caption:c.hub[2],alt:'AstraPay KYC review hub with both tasks incomplete',src:screens.hubEmpty,hotspots:[{...taskHotspots.ktp,nextKey:'ktp-first',event:'kyc_autosave_ktp'},{...taskHotspots.selfie,nextKey:'selfie-first',event:'kyc_autosave_selfie'}]},
    {key:'ktp-first',navNumber:'03',counter:'03 / 07',navGroup:'ktp',label:c.ktp[0],annotation:c.ktp[1],caption:c.ktp[2],alt:'AstraPay KTP capture screen',src:screens.ktpCapture,nextKey:'hub-ktp',event:'kyc_autosave_ktp'},
    {key:'hub-ktp',navNumber:'04',counter:'04 / 07',navGroup:'autosave',label:c.saved[0],annotation:`02 · ${c.saved[1]}`,caption:c.saved[2],alt:'AstraPay Tinjauan screen with KTP saved and Selfie incomplete',src:screens.hubKtp,hotspots:[{...taskHotspots.selfie,nextKey:'selfie-after-ktp',event:'kyc_autosave_selfie'}],event:'kyc_autosave_return'},
    {key:'selfie-after-ktp',navNumber:'05',counter:'05 / 07',navGroup:'selfie',label:c.selfie[0],annotation:`03 · ${c.selfie[1]}`,caption:c.selfie[2],alt:'AstraPay selfie liveness screen',src:screens.selfieCapture,nextKey:'hub-complete',event:'kyc_autosave_selfie'},
    {key:'selfie-first',counter:'03 / 07',navGroup:'selfie',showInNav:false,label:c.selfie[0],annotation:c.ktp[1],caption:locale === 'id' ? 'Flow yang sama juga mendukung Selfie lebih dulu, jadi urutan task memang fleksibel.' : 'The same system also supports Selfie first, so task order is genuinely flexible.',alt:'AstraPay selfie liveness screen',src:screens.selfieCapture,nextKey:'hub-selfie',event:'kyc_autosave_selfie'},
    {key:'hub-selfie',counter:'04 / 07',navGroup:'autosave',showInNav:false,label:c.saved[0],annotation:`02 · ${c.saved[1]}`,caption:c.saved[2],alt:'AstraPay Tinjauan screen with Selfie saved and KTP incomplete',src:screens.hubSelfie,hotspots:[{...taskHotspots.ktp,nextKey:'ktp-after-selfie',event:'kyc_autosave_ktp'}],event:'kyc_autosave_return'},
    {key:'ktp-after-selfie',counter:'05 / 07',navGroup:'ktp',showInNav:false,label:c.ktp[0],annotation:`03 · ${c.selfie[1]}`,caption:locale === 'id' ? 'KTP dapat diselesaikan setelah Selfie tanpa mengulang task yang sudah selesai.' : 'KTP can be completed after Selfie without repeating the finished task.',alt:'AstraPay KTP capture screen',src:screens.ktpCapture,nextKey:'hub-complete',event:'kyc_autosave_ktp'},
    {key:'hub-complete',navNumber:'06',counter:'06 / 07',navGroup:'complete',label:c.complete[0],annotation:`04 · ${c.complete[1]}`,caption:c.complete[2],alt:'AstraPay Tinjauan screen with KTP and Selfie completed',src:screens.hubComplete,nextKey:'processing',event:'kyc_autosave_complete'},
    {key:'processing',navNumber:'07',counter:'07 / 07',navGroup:'processing',label:c.processing[0],annotation:c.processing[1],caption:c.processing[2],alt:'AstraPay Upgrade Akun processing screen',src:screens.processing,isEnd:true,event:'kyc_autosave_submit'},
  ]
}

const presets = {
  'kyc-autosave-flow-b-en': {analyticsPrefix:'kyc_autosave_revamp', steps:makeSteps('en')},
  'kyc-autosave-flow-b-id': {analyticsPrefix:'kyc_autosave_revamp', steps:makeSteps('id')},
}

export function getPrototypePreset(name) {
