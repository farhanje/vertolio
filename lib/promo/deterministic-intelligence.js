...
function deterministicEvidence(fields, text, publishability) {
  ...
  if (Array.isArray(fields.paymentMethods) && fields.paymentMethods.length) {
    evidence.paymentMethods = evidenceLine(text, /qris|mybca|bca mobile|kartu kredit|kartu debit|sakuku|reward bca/i)
    confidence.paymentMethods = evidence.paymentMethods ? 0.95 : 0.7
  }
  const locationEvidence = evidenceLine(text, /berlaku di|tersedia di|seluruh outlet|semua outlet|online|website|aplikasi|mall|plaza|cabang|gerai/i)
  if (locationEvidence) {
...
export function enrichPromoDeterministically(extracted, context = {}) {
  ...
  const segmentation = deterministicSegmentation(extracted)
