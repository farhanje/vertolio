...
async function queueReview(sb, item, reasonsInput) {
  const reasons = unique((reasonsInput || []).map((field) => `ai_unresolved:${field}`))
...
function statusFromCalculated(fields, calculated) {
  if (calculated.expiryStatus === 'expired') return 'expired'
  if (calculated.expiryStatus === 'expiring_soon') return 'expiring_soon'
  if (fields.startsAt && new Date(fields.startsAt) > new Date()) return 'upcoming'
  return 'active'
}
...
async function applyResolvedItem(sb, item, value, llmResponse) {
  ...
  const calculated = calculatePromotionValue(fields)
  const expired = calculated.expiryStatus === 'expired'
  ...
  const requiresReview = !expired && (publishability.status === 'unresolved' || !validation.valid)
  const finalPublishabilityStatus = expired || !validation.valid
    ? 'review_required'
    : publishability.status
  const finalMissing = unique([
  ...
  await finishQueueItem(sb, item, {
    ...
    completed_at: new Date().toISOString(),
    result: value || {},
    last_error: requiresReview ? `Unresolved after AI: ${finalMissing.join(', ')}` : null,
  })
  if (requiresReview) await queueReview(sb, item, finalMissing)
  return {resolved: !requiresReview, publishabilityStatus: finalPublishabilityStatus}
}
...
export async function processNextPromoAiResolutionBatch({minimumAgeMinutes = 0} = {}) {
  ...
  const hydratedIds = new Set(items.map((item) => item.queue.id))
  for (const row of claim.data) {
    if (hydratedIds.has(row.id)) continue
    await sb.from('promo_ai_resolution_queue').update({
      status: 'cancelled',
      completed_at: new Date().toISOString(),
      last_error: 'Promotion or source no longer exists',
    }).eq('id', row.id)
  }
  ...
  return {
    ...
    unresolved: items.length - resolvedCount - failedCount,
    failed: failedCount,
