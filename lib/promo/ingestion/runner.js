...
import {
  augmentDiscoveryWithLegacyRows,
  clearPendingReviewForCanonical,
  deleteLegacyPromotionAfterGone,
...
function discoverySignature(discovered) {
  return createHash('sha256')
    .update((discovered || []).map((item) => String(item?.canonicalUrl || item?.url || '')).join('\n'))
    .digest('hex')
}
...
async function completeJob(sb, job, source, counters, logs, startedAt, batchState) {
  ...
  const {error: jobError} = await sb
    ...
    .update({
      ...
      records_deterministic: counters.deterministic,
      records_ai_queued: counters.aiQueued,
      execution_logs: logs,
...
export async function processNextPromoJob() {
  ...
  try {
    ...
    const sourceDiscovered = await adapter.discoverPromotionUrls()
    const discovered = await augmentDiscoveryWithLegacyRows(sb, source, sourceDiscovered)
    ...
    logs.push(logEntry('info', 'Promotion URLs discovered', {
      sourceCount: sourceDiscovered.length,
      totalWithMaintenance: discovered.length,
    ...
    for (const item of batchState.items) {
      try {
        const document = await adapter.fetchPromotion(item)
        await clearPendingReviewForCanonical(sb, source.id, document.canonicalUrl)
        ...
        if (outcome.aiQueued) counters.aiQueued += 1
        if (['created','updated'].includes(outcome.result) && !outcome.aiEnriched) counters.deterministic += 1
      } catch (error) {
        const removedGoneLegacy = await deleteLegacyPromotionAfterGone(sb, item, error)
        if (removedGoneLegacy) {
          counters.deleted += 1
          counters.materialChanges += 1
          logs.push(logEntry('info', 'Removed legacy promotion whose official page is gone', {
            url: item.url,
          }))
          continue
        }
    ...
    if (!batchState.hasMore) {
      const snapshotDeleted = await reconcileAuthoritativeSnapshot(sb, source, sourceDiscovered)
      if (snapshotDeleted) {
        counters.deleted += snapshotDeleted
        counters.materialChanges += snapshotDeleted
        logs.push(logEntry('info', 'Removed offers missing from authoritative source snapshot', {
    ...
    logs.push(logEntry('info', 'Deterministic-first promo summary', {
      created: counters.created,
      updated: counters.updated,
      unchanged: counters.unchanged,
      deterministic: counters.deterministic,
      aiQueued: counters.aiQueued,
      immediateAiCalls: counters.llmCalled,
