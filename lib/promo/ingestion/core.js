...
import { enrichPromoIntelligence } from '../intelligence'
...
function buildDuplicateFingerprint(fields) {
  ...
  if (!merchant || (!fields.benefitValue && !fields.voucherCode)) return null
...
function verificationStatus(fields, validation, duplicate) {
...
export async function processExtractedPromotion(sb, source, job, rawExtracted) {
  ...
  const hasFullIntelligence = ['gemini', 'cache'].includes(existing?.intelligence_method)
  const priorStatusSatisfied = signatureMatches
    && hasFullIntelligence
    && ['success', 'cached'].includes(existing?.segmentation_llm_status)
  const lastAttemptAt = existing?.segmentation_last_attempt_at
    ? new Date(existing.segmentation_last_attempt_at).getTime()
    : 0
  const manualRetry = job?.trigger_type === 'administrator_retry'
  const recentAttempt = !manualRetry
    && signatureMatches
  ...
  const extracted = await enrichPromoIntelligence(rawExtracted, {
    sb,
    source,
    job,
  ...
  if (extracted.extractedFields.isPromotion === false) {
    ...
    return {
      result: 'notPromotion',
      review: false,
      materialChange: Boolean(existing),
      llmStatus: currentLlmStatus,
      notPromotion: true,
      aiEnriched: ['success', 'cached'].includes(currentLlmStatus),
    }
  }
  ...
  if (calculated.expiryStatus === 'expired') {
    if (existing) {
      ...
      return {result: 'deleted', review: false, materialChange: true, llmStatus: currentLlmStatus, aiEnriched: ['success', 'cached'].includes(currentLlmStatus)}
    }
    return {result: 'expiredSkipped', review: false, materialChange: false, llmStatus: currentLlmStatus, aiEnriched: ['success', 'cached'].includes(currentLlmStatus)}
  }
  ...
  const verification = verificationStatus(extracted.extractedFields, validation, duplicate)
  const autoPublish = verification === 'verified' && shouldAutoPublish({source, extracted, validation})
  ...
  if (!existing) {
    ...
    if (verification === 'needs_attention') {
      await addReviewItem(sb, {documentId: document.id, promotionId: created.id, validation, extracted})
    }

    return {
      result: 'created',
      review: verification === 'needs_attention',
      materialChange: true,
      llmStatus: currentLlmStatus,
  ...
  const changes = detectMaterialChanges(previousFields(existing, existingOutlets), extracted.extractedFields)
