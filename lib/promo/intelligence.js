...
export const PROMO_INTELLIGENCE_VERSION = 'full-promo-v2'
...
const PROMO_INTELLIGENCE_SCHEMA = {
  ...
  required: [
    ...
    'evidence', 'warnings', 'contradictions',
  ],
  properties: {
...
function evidenceForField(result, field) {
  if (Array.isArray(result?.evidence)) {
    return result.evidence
      .filter((item) => item?.field === field)
      .sort((a, b) => safeConfidence(b?.confidence) - safeConfidence(a?.confidence))[0]?.quote || ''
  }
  return result?.evidence?.[field] || ''
}

function supportedValue(result, sourceText, evidenceKey, rawValue, fallback, transform = (value) => value) {
  if (!evidenceMatches(sourceText, evidenceForField(result, evidenceKey))) return fallback
...
function mergeOutlets(primary, fallback) {
  ...
  for (const outlet of [...(primary || []), ...(fallback || [])]) {
    const key = `${outlet.outletName}|${outlet.address || ''}|${outlet.city || ''}`.toLowerCase()
    if (!outlet.outName && !outlet.outletName) continue
    if (seen.has(key)) continue
    seen.add(key)
    result.push(outlet)
  }
...
function buildEvidenceMaps(result, sourceText) {
  const fieldConfidence = Object.fromEntries(FIELD_KEYS.map((key) => [key, 0]))
  const fieldEvidence = Object.fromEntries(FIELD_KEYS.map((key) => [key, '']))

  if (Array.isArray(result?.evidence)) {
    for (const item of result.evidence) {
      const key = item?.field
      const quote = normalizeText(item?.quote).slice(0, 500)
      const confidence = safeConfidence(item?.confidence)
      if (!FIELD_KEYS.includes(key) || !quote || !evidenceMatches(sourceText, quote)) continue
      if (confidence < fieldConfidence[key]) continue
      fieldConfidence[key] = confidence
      fieldEvidence[key] = quote
    }
  } else {
    for (const key of FIELD_KEYS) {
      const quote = normalizeText(result?.evidence?.[key]).slice(0, 500)
      if (!quote || !evidenceMatches(sourceText, quote)) continue
      fieldEvidence[key] = quote
      fieldConfidence[key] = safeConfidence(result?.fieldConfidence?.[key])
    }
  }

  return {fieldConfidence, fieldEvidence}
}

function sanitizeAiResult(result, sourceText, fallbackFields, deterministic) {
  if (!result || typeof result !== 'object') return null

  const {fieldConfidence, fieldEvidence} = buildEvidenceMaps(result, sourceText)
...
export async function enrichPromoIntelligence(extracted, context = {}) {
  ...
  const llmResponse = await runPromoLlmStructured({
    ...
    systemInstruction: [
      ...
      'For each supported field, add one evidence entry containing the field name, an exact quote from the source, and confidence from 0 to 1. Omit evidence for unsupported fields and leave their values empty or null.',
