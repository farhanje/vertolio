...
const PROMO_SEGMENTATION_SCHEMA = {
  type: 'object',
...
function explicitLocations(text) {
  ...
  for (const [city, province, aliases] of CITY_ALIASES) {
    if (city === 'Tangerang' && /\btangerang selatan\b/i.test(normalized)) continue
...
function inferCategories(text) {
  const scored = CATEGORY_RULES
    .map(([category, regex], index) => ({
      category,
      index,
      score: (String(text).match(new RegExp(regex.source, `${regex.flags.includes('g') ? regex.flags : `${regex.flags}g`}`)) || []).length,
    }))
...
function inferTags(text) {
  ...
  if (/\bqris\b/i.test(value)) tags.push('qris')
...
export function deterministicSegmentation(extracted) {
...
function sourceContainsEvidence(sourceText, evidence) {
  ...
  return Boolean(value && value.length >= 3 && source.includes(value))
}

function sanitizeLlmResult(value, sourceText) {
  if (!value || typeof value !== 'object') return null
  const explicit = explicitLocations(sourceText)
  ...
  const allowedCities = new Set(explicit.cities)
  const allowedProvinces = new Set(explicit.provinces)
  const cities = unique((value.cities || []).map(normalizeText).filter((item) => allowedCities.has(item)))
  const provinces = unique((value.provinces || []).map(normalizeText).filter((item) => allowedProvinces.has(item)))
  ...
  const outlets = (Array.isArray(value.outlets) ? value.outlets : []).map((item) => ({
    ...
    if (!item.outletName || !item.sourceText) return false
    if (!sourceContainsEvidence(sourceText, item.sourceText)) return false
    if (item.city && !allowedCities.has(item.city)) item.city = null
    if (item.province && !allowedProvinces.has(item.province)) item.province = null
    return true
  }).slice(0, 150)

  const requestedScope = LOCATION_SCOPES.includes(value.locationScope) ? value.locationScope : 'unknown'
  const locationScope = requestedScope === 'outlet' && !outlets.length
    ? inferLocationScope(sourceText, cities, provinces, outlets)
    : requestedScope
...
function shouldUseLlm(config, deterministic, extracted) {
  if (!config.enabled || config.mode === 'rules_only') return false
  if (config.mode === 'new_changed') return true

  const text = String(extracted?.rawRelevantText || extracted?.extractedFields?.termsText || '')
  return deterministic.primaryCategory === 'other'
    || deterministic.locationScope === 'unknown'
    || (OUTLET_CUES.test(text) && deterministic.outlets.length === 0)
    || Number(extracted?.extractionConfidence || 0) < 0.85
}

export async function enrichExtractedPromotion(extracted, context = {}) {
  ...
  if (!sourceText || !shouldUseLlm(config, deterministic, extracted)) {
    return {
      ...extracted,
      llmProcessing: {
        status: config.enabled ? 'not_needed' : (config.mode === 'rules_only' ? 'rules_only' : 'disabled'),
  ...
  const llmResponse = await runPromoLlmStructured({
    ...
    maxOutputTokens: 900,
    systemInstruction: [
      'You classify Indonesian public promotions into a strict taxonomy and extract explicit geographic coverage.',
      ...
      'For every outlet, copy the exact supporting phrase into sourceText.',
    ...
    schema: PROMO_SEGMENTATION_SCHEMA,
  })

  const llm = sanitizeLlmResult(llmResponse.result, sourceText)
  const merged = llm ? {
  ...
  return {
    ...extracted,
    llmProcessing: {
      status: llm ? llmResponse.status : (llmResponse.status || 'failed'),
