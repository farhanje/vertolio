...
export const PROMO_LLM_PROMPT_VERSION = 'full-promo-extraction-v3'
...
async function callGemini(config, request) {
  ...
  try {
    ...
    if (!text) {
      ...
      throw new Error(`Gemini returned no structured text output${blockReason ? `; prompt=${blockReason}` : ''}${finishReason ? `; finish=${finishReason}` : ''}`)
    }
