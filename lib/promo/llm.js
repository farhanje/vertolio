...
export function getPromoLlmConfig() {
  ...
  return {
    ...
    maxOutputTokens: Math.floor(numberFromEnv('PROMO_LLM_MAX_OUTPUT_TOKENS', DEFAULT_MAX_OUTPUT_TOKENS, {min: 100, max: 4000})),
...
function estimateTokens(text) {
  return Math.max(1, Math.ceil(String(text || '').length * 1.15))
}
...
async function callGemini(config, request) {
  ...
  try {
    ...
    if (!response.ok) {
      const detail = data?.error?.message || data?.message || `Gemini returned HTTP ${response.status}`
    ...
    const outputTokens = Number(usage.total_output_tokens || 0) + Number(usage.total_thought_tokens || 0)
    ...
    return {
      ...
      modelVersion: data.model || config.model,
    }
  } finally {
...
export async function runPromoLlmStructured({
  ...
  const outputLimit = Math.min(maxOutputTokens || config.maxOutputTokens, config.maxOutputTokens)
...
export async function testPromoLlmConnection(sb) {
  ...
  const result = await runPromoLlmStructured({
    ...
    bypassCache: true,
    maxOutputTokens: 120,
    systemInstruction: 'Return only the requested structured classification. Do not add facts that are not in the input.',
