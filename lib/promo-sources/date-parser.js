...
export const PROMO_DATE_PARSER_VERSION = 'evidence-date-v2'
...
function finalize({startsAt = null, expiresAt = null, evidence = {}, strategy = 'none', explicitRange = false}) {
  const anomalies = []
  ...
  if (comparison !== null && comparison > 0) {
    safeStart = null
    anomalies.push('start_after_expiry_cleared')
  } else if (comparison === 0 && !explicitRange) {
    safeStart = null
    anomalies.push('duplicate_expiry_not_used_as_start')
  }
...
export function findDateRange(text) {
  ...
  for (const line of lines) {
    ...
    return finalize({
      ...range,
      evidence: {startsAt: line, expiresAt: line},
